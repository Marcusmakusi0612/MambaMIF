...
from vmamba_Fusion_efficross import PatchEmbed2D
...
def train_fusion(num=0, logger=None):
    ...
    train_dataset = Fusion_dataset('train',length=15)
    ...
    for epo in range(0, epoch):
        ...
        for it, (image_vis, image_ir, name) in enumerate(train_loader):
            ...
            print(name)
            print(it)
            image_vis = torch.mean(image_vis, dim=4)
            print(image_vis.shape)
            try:
                fusionmodel.train()
            ...
            lossolt.append(loss_total.item())
            loss_total.backward()
            optimizer.step()
            ed = time.time()
